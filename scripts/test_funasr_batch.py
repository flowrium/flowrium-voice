...
def print_summary(report):
    ...
    print(
        ...
        f"norm_exact_match_rate={overall['norm_exact_match_rate']:.4f} "
        f"cer={overall['cer']:.4f} "
        f"avg_final_latency_ms={overall['avg_final_latency_ms']} "
        f"p95_final_latency_ms={overall['p95_final_latency_ms']}"
    )

    print("\nBy version")
    for key, value in summary["by_version"].items():
        print(
            f"- {key}: count={value['count']} "
            f"exact={value['exact_match_rate']:.4f} "
            f"norm_exact={value['norm_exact_match_rate']:.4f} "
            f"cer={value['cer']:.4f}"
        )
