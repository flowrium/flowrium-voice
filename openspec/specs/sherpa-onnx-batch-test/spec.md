...
### Requirement: 批量测试脚本支持三模型
系统 SHALL 提供 `scripts/test_sherpa_onnx_batch.py` 脚本，支持通过 `--model` 参数选择 Paraformer Int8、SenseVoice Int8 或 Transducer Int8 模型。
...
#### Scenario: 使用 Transducer 模型测试
- **WHEN** 执行 `python test_sherpa_onnx_batch.py --model transducer`
- **THEN** 脚本加载 Transducer Int8 模型，对 manifest 中所有音频进行推理，输出 CER 和 RTF 指标
...
### Requirement: 热词测试
脚本 SHALL 支持 `--hotword` 参数，对 Transducer 模型加热词进行测试。Paraformer 和 SenseVoice 模型忽略热词参数并给出提示。

#### Scenario: Transducer 加热词测试
- **WHEN** 执行 `python test_sherpa_onnx_batch.py --model transducer --hotword 出勤率 --hotword 合格率 --decoding-method modified_beam_search`
...
#### Scenario: 热词文件加载
- **WHEN** 执行 `python test_sherpa_onnx_batch.py --model transducer --use-hotwords-file`
- **THEN** 脚本从 `config/hotwords.txt` 读取热词，并在推理时启用热词偏置
