...
TEST_SCRIPT="$ROOT_DIR/verification-3-edge/whisper.cpp/scripts/test_whisper_cpp_batch.py"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --help|-h)
      cat <<'EOF'
Usage: bash verification-3-edge/whisper.cpp/benchmarks/apple/run_macos_benchmark.sh [options] [-- script args]
EOF
      exit 0
      ;;
    --)
...
RESULT_JSON="$ROOT_DIR/verification-3-edge/whisper.cpp/results/whisper-cpp-${MODEL}-no-hotword-results.json"
